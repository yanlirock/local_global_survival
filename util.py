...
class Surv:
    """
    Helper class to construct structured array of
    ...
    def from_arrays(event, time, name_event=None, name_time=None):
        ...
        y = numpy.empty(time.shape[0],
                        dtype=[(name_event, numpy.bool_),
                               (name_time, numpy.float_)])
        y[name_time] = time
        ...
        if numpy.issubdtype(event.dtype, numpy.bool_):
            ...
            if numpy.all(events == numpy.array([0, 1], dtype=events.dtype)):
                ...
                raise ValueError('non-boolean event indicator '
                                 'must contain 0 and 1 only')
        return y
...
def check_y_survival(y_or_event, *args, allow_all_censored=False):
    ...
    *args : list of array-likes
    ...
    if len(args) == 0:
        ...
        if not isinstance(y, numpy.ndarray) or y.dtype.fields is None or \
                len(y.dtype.fields) != 2:
            raise ValueError(
    ...
    if not numpy.issubdtype(event.dtype, numpy.bool_):
        raise ValueError('elements of event indicator must be boolean, '
                         'but found {0}'.format(event.dtype))
    ...
    for i, yt in enumerate(time_args):
        ...
        if not numpy.issubdtype(yt.dtype, numpy.number):
            raise ValueError('time must be numeric, but found {} for '
                             'argument {}'.format(yt.dtype, i + 2))
    ...
    for df in objs:
        if isinstance(df, pandas.Series):
            if is_categorical_dtype(df.dtype):
                categories[df.name] = {"categories": df.cat.categories,
            ...
            for name, s in dfc.iteritems():
                if name in categories:
                    ...
                    if not categories[name]["categories"].equals(
                            s.cat.categories):
                        raise ValueError(
                            "categories for column %s do not match" % name)
                else:
                    categories[name] = {"categories": s.cat.categories,
